...
public class SortTypes {
    // Comparator to place blocks first
    ...
    // Comparator to place items first (non-block items)
    ...
    // Comparator to place stackable items first
    ...
    // Comparator to place unstackable items first
    ...
    // Comparator to match items by a specific Identifier (item ID)
    ...
    // Comparator to match items by a specific tag
    ...
    // Comparator to match blocks by a specific tag
    public static Comparator<ItemStack> blockTag(Identifier id) {
        return (lhs, rhs) -> {
            ...
            if (lItem instanceof BlockItem && Block.getBlockFromItem(lItem).getDefaultState().isIn(TagKey.of(RegistryKeys.BLOCK, id))) {
                l = 0;
            }
            if (rItem instanceof BlockItem && Block.getBlockFromItem(rItem).getDefaultState().isIn(TagKey.of(RegistryKeys.BLOCK, id))) {
                r = 0;
            }
            return l - r;
        };
    ...
    // Comparator to match items by a specific item group
    ...
    // Comparator to match items by their order within a specific item group
    ...
    // Comparator to place tools first
    ...
    // Comparator to place armor first
    ...
    // Comparator to sort by item damage (ascending order)
    public static Comparator<ItemStack> damage = Comparator.comparingInt(ItemStack::getDamage);

    // Comparator to sort by item count (descending order)
